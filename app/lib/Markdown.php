...
class Markdown implements MarkdownInterface{
	 ...
	 * @return mixed|string
	 */
	protected function encodeUrlAttribute($text){
		$text = $this->encodeAttribute($text);
		if(!filter_var($text, FILTER_VALIDATE_URL)){
			$text = "#";
		}
		return hook()->apply('Markdown_encodeUrlAttribute', $text);
	}
	...
	protected function _doAutoLinks_tel_callback($matches){
		$url = $this->encodeUrlAttribute($matches[1]);
		$tel = $this->encodeUrlAttribute($matches[2]);
	...
	protected function _hashHTMLBlocks_inMarkdown($text, $indent = 0, $enclosing_tag_re = '', $span = false){
		...
		do{
			...
			#
				...
				[$block_text, $text] = $this->_hashHTMLBlocks_inHTML($tag . $text, "hashBlock", true);
			...
			#
				...
				[$block_text, $text] = $this->_hashHTMLBlocks_inHTML($tag . $text, "hashClean", false);
			...
			#
			else if($enclosing_tag_re !== '' && # Same name as enclosing tag.
		...
		do{
			...
			if(preg_match('{^</?(?:' . $this->auto_close_tags_re . ')\b}', $tag) || $tag[1] == '!' || $tag[1] == '?'){
				...
				if($md_attr && preg_match($markdown_attr_re, $tag, $attr_m) && preg_match('/^1|block|span$/', $attr_m[2] . $attr_m[3])){
					...
					[$block_text, $text] = $this->_hashHTMLBlocks_inMarkdown($text, $indent, $tag_name_re, $span_mode);
